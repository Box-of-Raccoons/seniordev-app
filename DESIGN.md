...
SeniorDev is a dark, legible operations surface where a developer composes and
supervises machine work. A single full-height region carries a row of tabs, and
each tab is one session you launched. A new tab opens as an inline composer (pick
a mode, a folder, a role, and type a ticket key or a sentence of intent), then on
launch it morphs in place into a live CLI-agent terminal. Every autonomous run is
watchable and traceable: a scrolling log, tool one-liners, PR cards, a structured
recap. The room is dim so the work is bright. Surfaces are a warm near-neutral
charcoal (a faint teal-green cast at chroma ~0.012), text is a warm off-white, and
a single restrained accent, a muted teal, marks the one action that matters on any
given surface. Color is a signal here, never decoration.
...
mascot is warmth reserved for moments like splash, empty states, and About, never
a theme smeared across the working surface), the maximal IDE (no cluttered
...
- **Courier Teal** (`oklch(0.78 0.085 168)`): The one accent. Primary buttons
  (the composer's `Launch`, `Resume`, PR `Open`) and focus outlines. Muted and
  desaturated so it signals without shouting; its rarity is the point.
...
- **Alarm Rust** (`oklch(0.68 0.15 38)`): Errors: the composer's error line, failure
...
- **Room Charcoal (bg)** (`oklch(0.21 0.012 165)`): The deepest plane. App
  background and terminal shells.
- **Panel Charcoal (surface)** (`oklch(0.255 0.014 165)`): Composer fields and
  input surfaces; one tonal step up from `bg`.
...
at consistent DPI; a fluid heading that shrinks in a narrow tab is a regression,
not a feature.
...
by lightness, from `bg` (0.21) to `surface` (0.255) to `surface-2` (0.305), and
hairline white-alpha borders draw the seams between them. This tonal layering does most of
...
lift and overlay, not for separating adjacent surfaces; that's the hairline's job.
...
- **Style:** Top-rounded (`8px 8px 0 0`), `surface` background, `ink-soft` label,
  hairline border with no bottom edge, `5px 10px` padding, so tabs sit into the
  surface below them.
...
- **Do** give every interactive element a visible `outline` focus state; the tool
  is keyboard-first.
- **Do** honor `prefers-reduced-motion` on every animation (the composer
  transition and empty states already do) with a crossfade or instant fallback.
