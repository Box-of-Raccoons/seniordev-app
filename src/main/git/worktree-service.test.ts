...
describe('gitRepoInfo', () => {
  it('is a repo when rev-parse prints true', async () => {
    const r = fakeRunner({ 'rev-parse --is-inside-work-tree': { code: 0, stdout: 'true\n', stderr: '' } })
    expect((await gitRepoInfo(r, '/x')).isRepo).toBe(true)
  })
  it('is not a repo on a non-zero exit', async () => {
    const r = fakeRunner({ 'rev-parse --is-inside-work-tree': { code: 128, stdout: '', stderr: 'not a git repository' } })
    expect((await gitRepoInfo(r, '/x')).isRepo).toBe(false)
  })
})

describe('createWorktree', () => {
  it('runs "worktree add -b <branch> <path> HEAD" and returns the path', async () => {
    const r = fakeRunner({ 'worktree add': { code: 0, stdout: '', stderr: '' } })
    const res = await createWorktree(r, { configDir: '/cfg', folder: '/repo', repoKey: 'repo', branch: 'hardy/add-thing' })
  ...
  it('sanitizes the branch before use (empty -> task)', async () => {
    const r = fakeRunner({ 'worktree add': { code: 0, stdout: '', stderr: '' } })
    const res = await createWorktree(r, { configDir: '/cfg', folder: '/repo', repoKey: 'repo', branch: '' })
    expect(res.ok && res.branch).toBe('task')
    expect(r.calls[0].args[3]).toBe('task')
  })
  it('maps a branch-exists collision to a clear error, cleaned of the git prefix', async () => {
    const r = fakeRunner({ 'worktree add': { code: 128, stdout: '', stderr: "fatal: a branch named 'x' already exists\n" } })
    const res = await createWorktree(r, { configDir: '/cfg', folder: '/repo', repoKey: 'repo', branch: 'x' })
    expect(res).toEqual({ ok: false, error: "a branch named 'x' already exists" })
  })
  it('maps a path-exists collision to git\'s message', async () => {
...
describe('removeWorktree', () => {
  ...
  it('surfaces any other git error, cleaned', async () => {
